...
`gc` version `1.22.3` targeting FreeBSD running on AArch64.
...
the chain, `gc` `1.20`, then compiles `gc` `1.22.3` targeting $S_"target"$. The
...
#set par(justify: false)
#figure(
  caption: [A program that prints its own code, as generated by `evilgen`.]
)[
```go
package main
...
simple. Targeting this implementation in an attack should be very difficult,
